...
abstract class Optimization {
  ...
  static List<Operations> optimize(String code) {
    return optimize(Tokens.tokenize(code));
  }

  private static List<Operations> optimize(List<Operations> tokens) {
      ...
      switch (token.type) {
        ...
        case ZERO:
          ...
          if (retValue.peek().type != token.type) {
            ...
            if (retValue.peek().type == Operations.Type.ZERO) {
              retValue.peek().arg = 1; //убираем повторы
            }
